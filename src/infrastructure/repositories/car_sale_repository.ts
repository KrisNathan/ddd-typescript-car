...
import type { DatabaseOrTransaction } from "@infrastructure/database/types";
...
export default class CarSaleRepository implements ICarSaleRepository {
  ...
  async recordSale(carSale: CarSale): Promise<UUID> {
    const result = await this.db.insert(carSales).values({
  ...
  async getAllSales(): Promise<CarSale[]> {
    const rows = await this.db.select().from(carSales);
